...
public class TeamsJFrame extends JFrame implements ActionListener, ListSelectionListener
{
    ...
    {
        super("GDM's JRadioButton example");
        setSize(400, 300);
        setLocation(200, 200);
    ...
    {
        teamList = new JRadioButton[teams.size()];
        buttonGroup = new ButtonGroup();

        for (int i = 0; i < teamList.length; i++)
        {
            teamList[i] = new JRadioButton(teams.get(i).getName());
            teamList[i].setOpaque(false);
            teamList[i].addActionListener(this);
            buttonGroup.add(teamList[i]);
        }
    ...
    private JPanel setupCheckBoxPane(JRadioButton[] teamList)
    {
        JPanel p = new JPanel(new GridLayout(teamList.length, 1, 5, 5));
        p.setOpaque(false);
        for (JRadioButton b : teamList)
        {
            p.add(b);
        }
        return p;
